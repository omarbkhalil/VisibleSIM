#ifndef catomMAPFCBS1BlockCode_H_
#define catomMAPFCBS1BlockCode_H_
...
class catomMAPFCBS1BlockCode : public Catoms3DBlockCode {
private:
...
public :
    ...
    std::vector<Cell3DPosition> planSingleAgentWithConstraints(
            int agentId,
            const Cell3DPosition& start,
            const Cell3DPosition& goal,
            const ConstraintSet& constraints);

    bool isMovableNow(const Cell3DPosition &pos);

    // New: low-level planner that respects constraints (CBS leaf planner)

    // Optional: small adapter to bind into CBS
...
int reverseIndex = -1;
    ...
    catomMAPFCBS1BlockCode(Catoms3DBlock *host);
    ~catomMAPFCBS1BlockCode() {};
...
//    std::array<Cell3DPosition, 12> getTargetFBPlus16X() const;
   ...
   shiftPlusX(const std::array<Cell3DPosition, N>& src, int dx) {
    ...
    void recomputeCBSForRemaining(const Cell3DPosition &origin, const std::vector<Cell3DPosition> &starts,
                                  const std::vector<Cell3DPosition> &targets);

    void launchNextAgent(const Cell3DPosition &origin, const std::vector<Cell3DPosition> &starts,
                         const std::vector<Cell3DPosition> &targets);
    ...
    static BlockCode *buildNewBlockCode(BuildingBlock *host) {
        return (new catomMAPFCBS1BlockCode((Catoms3DBlock*)host));
    };
};

#endif /* catomMAPFCBS1BlockCode_H_ */
