...
map<Cell3DPosition, vector<Cell3DPosition>> CatomsTest6BlockCode::cells;
vector<Cell3DPosition> CatomsTest6BlockCode::visited;
vector<Cell3DPosition> CatomsTest6BlockCode::teleportedPositions;
...
> CatomsTest6BlockCode::openSet{
    [](std::pair<Cell3DPosition, double> a, std::pair<Cell3DPosition, double> b) {
        return a.second > b.second;
    }
};

std::map<Cell3DPosition, double> CatomsTest6BlockCode::gScore;
std::map<Cell3DPosition, double> CatomsTest6BlockCode::fScore;
map<Cell3DPosition, Cell3DPosition> CatomsTest6BlockCode::cameFrom;
set<Cell3DPosition> CatomsTest6BlockCode::closedSet;

std::queue<Cell3DPosition> CatomsTest6BlockCode::startQueue(
    std::deque{
...
std::queue<Cell3DPosition> CatomsTest6BlockCode::targetQueue(
    std::deque{
...
CatomsTest6BlockCode::CatomsTest6BlockCode(Catoms3DBlock *host) : Catoms3DBlockCode(host) {
    if (!host) return;
    module = static_cast<Catoms3DBlock*>(hostBlock);
}

void CatomsTest6BlockCode::startup() {
    console << "start\n";
...
void CatomsTest6BlockCode::onMotionEnd() {
    console << " has reached its destination\n";
}

void CatomsTest6BlockCode::processLocalEvent(EventPtr pev) {
...
void CatomsTest6BlockCode::onBlockSelected() {
    cerr << endl << "--- PRINT MODULE " << *module << "---" << endl;
}

void CatomsTest6BlockCode::onAssertTriggered() {
    console << " has triggered an assert\n";
}

bool CatomsTest6BlockCode::parseUserCommandLineArgument(int &argc, char **argv[]) {
    if ((argc > 0) && ((*argv)[0][0] == '-')) {
        switch((*argv)[0][1]) {
            case 'b': {
...
std::string CatomsTest6BlockCode::onInterfaceDraw() {
    std::stringstream trace;
    trace << "Distance: " << distance;
    return trace.str();
}

double CatomsTest6BlockCode::heuristic(const Cell3DPosition& current, const Cell3DPosition& goal) {
