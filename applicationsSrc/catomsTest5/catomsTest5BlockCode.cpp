#include "CatomsTest5BlockCode.hpp"
...
std::map<Cell3DPosition, std::vector<Cell3DPosition> > CatomsTest5BlockCode::cells;
std::vector<Cell3DPosition> CatomsTest5BlockCode::visited;
std::vector<Cell3DPosition> CatomsTest5BlockCode::teleportedPositions;
...
> CatomsTest5BlockCode::openSet{
    [](std::pair<Cell3DPosition, double> a, std::pair<Cell3DPosition, double> b) {
        return a.second > b.second;
    }
};

std::map<Cell3DPosition, double> CatomsTest5BlockCode::gScore;
std::map<Cell3DPosition, double> CatomsTest5BlockCode::fScore;
std::map<Cell3DPosition, Cell3DPosition> CatomsTest5BlockCode::cameFrom;
std::set<Cell3DPosition> CatomsTest5BlockCode::closedSet;

std::vector<std::vector<Cell3DPosition> > CatomsTest5BlockCode::globalOptimalPaths;

//Origin/Relative pos
Cell3DPosition CatomsTest5BlockCode::Origin = Cell3DPosition(17 + 18, 5, 6);


// Queues for start and target positions.
std::queue<Cell3DPosition> CatomsTest5BlockCode::startQueue(
    std::deque{
...
//state 1
std::queue<Cell3DPosition> CatomsTest5BlockCode::targetQueue(
    std::deque{
...
CatomsTest5BlockCode::CatomsTest5BlockCode(Catoms3DBlock *host) : Catoms3DBlockCode(host) {
    if (!host) return;
    module = static_cast<Catoms3DBlock *>(hostBlock);
}


void CatomsTest5BlockCode::startup() {
    console << "start\n";
...
void CatomsTest5BlockCode::onMotionEnd() {
    console << " has reached its destination\n";
}

void CatomsTest5BlockCode::processLocalEvent(EventPtr pev) {
    // Normal A* processing.
...
void CatomsTest5BlockCode::onBlockSelected() {
    std::cerr << std::endl << "--- PRINT MODULE " << *module << " ---" << std::endl;
}

void CatomsTest5BlockCode::onAssertTriggered() {
    console << " has triggered an assert\n";
}

bool CatomsTest5BlockCode::parseUserCommandLineArgument(int &argc, char **argv[]) {
    if ((argc > 0) && ((*argv)[0][0] == '-')) {
        switch ((*argv)[0][1]) {
            case 'b': {
...
std::string CatomsTest5BlockCode::onInterfaceDraw() {
    std::stringstream trace;
    trace << "Distance: " << distance;
    return trace.str();
}

double CatomsTest5BlockCode::heuristic(const Cell3DPosition &current, const Cell3DPosition &goal) {
    return std::abs(current[0] - goal[0]) +
           std::abs(current[1] - goal[1]) +
           std::abs(current[2] - goal[2]);
}

void CatomsTest5BlockCode::saveOptimalPath(const std::vector<Cell3DPosition> &path) {
...
void CatomsTest5BlockCode::initiateNextModulePathfinding() {
    if (startQueue.empty()) {
