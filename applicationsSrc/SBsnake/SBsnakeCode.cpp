...
SBsnakeCode::SBsnakeCode(SlidingCubesBlock *host):SlidingCubesBlockCode(host),module(host) {
    // @warning Do not remove block below, as a blockcode with a NULL host might be created
    //  for command line parsing
    if (not host) return;

    addMessageEventFunc2(GO_MSG_ID,
                       std::bind(&SBsnakeCode::myGoFunc,this,
                       std::placeholders::_1, std::placeholders::_2));

}

void SBsnakeCode::startup() {
    console << "start " << getId() << "\n";
...
void SBsnakeCode::parseUserBlockElements(TiXmlElement *config) {
...
void SBsnakeCode::parseUserElements(TiXmlDocument *config) {
...
void SBsnakeCode::myGoFunc(std::shared_ptr<Message>_msg, P2PNetworkInterface*sender) {
    int i = 0;
...
string SBsnakeCode::onInterfaceDraw(){
    if(isFinish){
        return "Goal reached!";
    }
    return "";
...
bool SBsnakeCode::isItObstacle(int id){
    return (find(obstacleIds.begin(), obstacleIds.end(), id) != obstacleIds.end());
}
bool SBsnakeCode::isItObstacle(Cell3DPosition pos){
