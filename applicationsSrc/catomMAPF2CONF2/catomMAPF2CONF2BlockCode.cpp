#include "CatomMAPF2CONF2BlockCode.hpp"
...
template<size_t N>
static std::vector<Cell3DPosition> vecFrom(const std::array<Cell3DPosition,N>& a) {
...
Cell3DPosition CatomMAPF2CONF2BlockCode::initiator  = Cell3DPosition(5, 1, 0);
Cell3DPosition CatomMAPF2CONF2BlockCode::initiator2  = Cell3DPosition(10, 3, 5);
Cell3DPosition CatomMAPF2CONF2BlockCode::initiator3  = Cell3DPosition(14, 4, 5);
Cell3DPosition CatomMAPF2CONF2BlockCode::initiator4  = Cell3DPosition(18, 3, 5);

Cell3DPosition CatomMAPF2CONF2BlockCode::Origin  = Cell3DPosition(4, 4, 2);
Cell3DPosition CatomMAPF2CONF2BlockCode::OriginB = Cell3DPosition(8, 4, 2);

static const std::array<Cell3DPosition, 16> FB_RELATIVE_POSITIONS = {

    // Manual ASSIGN and ORDER

    Cell3DPosition(1,1,1),
    Cell3DPosition(1,4,1),
    Cell3DPosition(1,2,1),
    Cell3DPosition(1,3,1),

    Cell3DPosition(2,1,1),
    Cell3DPosition(2,4,1),


    Cell3DPosition(2,3,1),
Cell3DPosition(2,2,1),





    Cell3DPosition(3,1,1),
    Cell3DPosition(3,4,1),
    Cell3DPosition(3,2,1),
    Cell3DPosition(3,3,1),


    Cell3DPosition(4,1,1),
    Cell3DPosition(4,4,1),

    Cell3DPosition(4,3,1),
    Cell3DPosition(4,2,1),
...
static const std::array<Cell3DPosition, 12> BF_RELATIVE_POSITIONS = {
      Cell3DPosition(-1, -1, 0), Cell3DPosition(-1, 0, 0),  Cell3DPosition(-1, +1, 0), Cell3DPosition(-1, +2, 0)
};

static std::array<Cell3DPosition, 16> gFBStartPositions;
static bool gFBStartsBuilt = false;

static std::array<Cell3DPosition, 16> gFBTargetPositions;
...
CatomMAPF2CONF2BlockCode::CatomMAPF2CONF2BlockCode(Catoms3DBlock *host) : Catoms3DBlockCode(host) {
    if (!host) return;

    addMessageEventFunc2(GO_ID,   std::bind(&CatomMAPF2CONF2BlockCode::handleGo,   this,
                                            std::placeholders::_1, std::placeholders::_2));
    addMessageEventFunc2(BACK_ID, std::bind(&CatomMAPF2CONF2BlockCode::handleBack, this,
                                            std::placeholders::_1, std::placeholders::_2));
    addMessageEventFunc2(FIRST_ID,
        std::bind(&CatomMAPF2CONF2BlockCode::handleFirst, this,
                  std::placeholders::_1, std::placeholders::_2));
    addMessageEventFunc2(REACHED_ID,
        std::bind(&CatomMAPF2CONF2BlockCode::handleReached, this,
                  std::placeholders::_1, std::placeholders::_2));
...
void CatomMAPF2CONF2BlockCode::startup() {
    if (module->position == initiator) {
        if (!gFBStartsBuilt) {
            resetPlansFile();
            for (size_t i = 0; i < FB_RELATIVE_POSITIONS.size(); ++i)
                gFBStartPositions[i] = FB_RELATIVE_POSITIONS[i];   // absolute positions
        ...
        if (!gFBTargetsBuilt) {
            ...
            for (size_t i = 0; i < FB_RELATIVE_POSITIONS.size(); ++i)
                gFBTargetPositions[i] = Cell3DPosition(
                    gFBStartPositions[i][0] + 10,
                    gFBStartPositions[i][1],
                    gFBStartPositions[i][2]
                );
...
void CatomMAPF2CONF2BlockCode::broadcastFirstTo(const Cell3DPosition& target,
                                         const Cell3DPosition& goal) {
...
void CatomMAPF2CONF2BlockCode::handleFirst(std::shared_ptr<Message> _msg, P2PNetworkInterface* sender) {
...
void CatomMAPF2CONF2BlockCode::handleReached(std::shared_ptr<Message> _msg, P2PNetworkInterface* sender) {
...
void CatomMAPF2CONF2BlockCode::handleGo(std::shared_ptr<Message> _msg, P2PNetworkInterface* sender) {
...
void CatomMAPF2CONF2BlockCode::handleBack(std::shared_ptr<Message> _msg, P2PNetworkInterface* sender) {
...
void CatomMAPF2CONF2BlockCode::onMotionEnd() {
    ...
    pathIdx++;
    if (pathIdx < (int)currentPath.size() &&
        currentPath[pathIdx] == module->position) {
        pathIdx++;
    }

    // If more waypoints remain → continue movement
    if (pathIdx < (int)currentPath.size()) {
        console << "[MOVE] Module " << module->blockId
                << " continuing step " << pathIdx
                << "/" << currentPath.size()
                << " → " << currentPath[pathIdx] << "\n";

        if (!scheduleGuardedHop()) {
            console << "[MOVE] Could not schedule hop; waiting.\n";
        }
        return;
    }

    // Path is fully completed
    console << "[MOVE] Module " << module->blockId
            << " reached final waypoint " << module->position << "\n";
    hasPath = false;
    ...
    // -------------------------------------------------------------
    //  NORMAL MODULES (non-initiator) → send REACHED + exit
    // -------------------------------------------------------------
    ...
    // =============================================================
    //  INITIATOR BEHAVIOR (the FIX)
    //  Initiator MUST:
    //  1) Report REACHED normally
    //  2) Launch next module
    //  3) Move itself if it has a path
    // =============================================================

    console << "[REACHED][ORIGIN] Initiator finished hop.\n";

    // 1) Send REACHED to itself (so window can refill)
    {
        CReachedPayload rp{
            initiator,          // origin
    ...
    // 2) If initiator still has a path → CONTINUE MOVING
    if (hasPath && pathIdx < (int)currentPath.size()) {
        console << "[ORIGIN] Continuing my own movement.\n";

        if (!scheduleGuardedHop()) {
            console << "[ORIGIN] hop blocked; retry pending.\n";
            gWaitingForRetry[module->blockId] = true;
        }
    }

    // 3) Launch next module (only when initiator is responsible)
    console << "[ORIGIN] Checking launch queue...\n";

    if (nextPairIdx < (int)gFBStartPositions.size()) {
        Cell3DPosition nextTarget = gFBStartPositions[nextPairIdx];
        Cell3DPosition nextGoal   = gFBTargetPositions[nextPairIdx];

        console << "[ORIGIN] Launching module index " << nextPairIdx
                << ": " << nextTarget << " → " << nextGoal << "\n";
    ...
    // =============================================================
    //   FB PHASE COMPLETION (only once all modules launched)
    // =============================================================
    console << "[ORIGIN] All FB modules have completed.\n";
    setColor(PURPLE);

    gReachedWave += 100;

    CReachedPayload finalMsg{
        initiator2,
        module->position,
        gReachedWave,
        true
    ...
    sendMessageToAllNeighbors(
        "REACHED_FINAL",
        new MessageOf<CReachedPayload>(REACHED_ID, finalMsg),
        1000, 100, 0
    );

    console << "[ORIGIN] Sent FINAL to initiator2.\n";
}
...
CatomMAPF2CONF2BlockCode::findPath(Cell3DPosition &start, Cell3DPosition &goal) {
...
// ---------------------------
bool CatomMAPF2CONF2BlockCode::getAllPossibleMotionsFromPosition(
...
void CatomMAPF2CONF2BlockCode::startFarthestWaveFromHere() {
...
// ---------------------------
bool CatomMAPF2CONF2BlockCode::parseUserCommandLineArgument(int &argc, char **argv[]) {
    (void)argc; (void)argv; // unused
    return false;
}

void CatomMAPF2CONF2BlockCode::processLocalEvent(EventPtr pev) {
    Catoms3DBlockCode::processLocalEvent(pev);
}

void CatomMAPF2CONF2BlockCode::onBlockSelected() {
}

void CatomMAPF2CONF2BlockCode::onAssertTriggered() {
}

std::string CatomMAPF2CONF2BlockCode::onInterfaceDraw() {
    return std::string();
}

bool CatomMAPF2CONF2BlockCode::scheduleGuardedHop() {
    gWaitingForRetry[module->blockId] = false;
